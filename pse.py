...
def pse(kernals, min_area):
    ...
    label, label_num = skimage.measure.label(kernals[kernal_num - 1], neighbors=4, return_num=True)#四邻域标签，返回
    ...
    dx = [-1, 1, 0, 0]
    dy = [0, 0, -1, 1]
    for kernal_idx in range(kernal_num - 2, -1, -1):#遍历每一个kernel
